...
function AdminLogin() {
  ...
  const handleNext = () => {
    if (step === 1 && !uid) return alert("Enter Admin ID");
    setStep(step + 1);
  };
...
const handleLogin = async () => {
  ...
  return (
    <div className="min-h-screen relative flex items-center justify-center">
      {/* Background */}
      <img src={adminImg} alt="admin background" className="absolute w-full h-full object-conver" />
      ...
      <div className="relative z-10 bg-white/20 backdrop-blur-md p-8 rounded-xl text-white w-80">
        ...
        <h2 className="text-xl mb-4 text-center">
          {uid ? `Admin Login – ID: ${uid}` : "Admin Login"}
        </h2>
